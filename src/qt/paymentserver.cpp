...
const QString MICRO_IPC_PREFIX("bitcoin:");
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(MICRO_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    else if (s.startsWith(MICRO_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
