...
Bitcoin Core 0.15.0 contains the following changes to the RPC interface and `micro-cli` for multi-wallet:

* When running Bitcoin Core with a single wallet, there are **no** changes to the RPC interface or `micro-cli`. All RPC calls and `micro-cli` commands continue to work as before.
...
- `uptime` returns the total runtime of the `microd` server since its last start (See [PR 10400](https://github.com/bitcoin/bitcoin/pull/10400)).
