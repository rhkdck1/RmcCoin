...
AC_DEFUN([MICRO_QT_CHECK],[
  if test "x$bitcoin_enable_qt" != xno && test "x$micro_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl Inputs: micro_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If micro_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: micro_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_MICRO_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  MICRO_QT_CHECK([
    if test "x$micro_qt_want_version" = xauto; then
      _MICRO_QT_CHECK_QT5
